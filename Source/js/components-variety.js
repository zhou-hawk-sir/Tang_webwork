        ...
        <div class="container">
          ...
          <div class="grid-3">
            <div class="card hover-lift fade-in-up" v-for="(show, i) in filteredItems" :key="i">
              <div style="height:140px;display:flex;align-items:center;justify-content:center;background:linear-gradient(135deg,#FF6B35,#D4A853);border-radius:var(--radius-md) var(--radius-md) 0 0;position:relative;">
                <span style="font-size:3rem;opacity:0.4;">·</span>
                <span class="card-tag" style="position:absolute;top:0.75rem;right:0.75rem;padding:0.2rem 0.6rem;border-radius:0;font-size:0.75rem;font-weight:600;color:#fff;background:rgba(0,0,0,0.4);">{{ show.type }}</span>
              </div>
              <div class="card-body">
            ...
            <div class="card hover-lift fade-in-up" v-for="(radio, i) in radios" :key="i">
              <div class="card-body" style="padding:2rem;">
                <div style="display:flex;gap:1.25rem;align-items:flex-start;">
                  <div style="width:64px;height:64px;border-radius:0;background:linear-gradient(135deg,#FF6B35,#D4A853);display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                    <span style="font-size:1.8rem;">·</span>
                  </div>
                  <div style="flex:1;">
                    <h4 style="font-size:1.15rem;margin-bottom:0.35rem;">{{ radio.title }}</h4>
                    <div style="display:flex;gap:0.75rem;margin-bottom:0.75rem;flex-wrap:wrap;">
                      <span class="card-tag" style="padding:0.15rem 0.5rem;border-radius:0;font-size:0.8rem;color:var(--primary);background:rgba(255,107,53,0.1);">{{ radio.host }}</span>
        ...
        <div class="container">
          <div class="grid-2">
            <div class="card hover-lift fade-in-up" v-for="(item, i) in filmTV" :key="i">
              <div style="height:200px;display:flex;align-items:center;justify-content:center;background:linear-gradient(135deg,#1a1a2e,#16213e);position:relative;border-radius:var(--radius-md) var(--radius-md) 0 0;">
              ...
              <div class="card-body">
                <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:0.5rem;">
                  ...
                  <span class="card-tag" style="padding:0.15rem 0.5rem;border-radius:0;font-size:0.75rem;color:var(--primary);background:rgba(255,107,53,0.1);">{{ item.type }}</span>
                </div>
        ...
        <div class="container">
          <div class="grid-3">
            <div class="card hover-lift fade-in-up" v-for="(doc, i) in documentaries" :key="i">
              <div style="height:180px;display:flex;align-items:center;justify-content:center;background:linear-gradient(135deg,#0f0c29,#302b63);position:relative;border-radius:var(--radius-md) var(--radius-md) 0 0;">
...
window.Components.Commercials = {
    ...
    <div class="page-content">
      <section class="page-banner" data-section="variety">
        <div class="container">
          ...
          <h1 class="banner-title">代言与官方身份</h1>
          <p class="banner-subtitle">品牌代言与官方荣誉</p>
        </div>
      </section>
      ...
      <section class="content-section">
        <div class="container">
          ...
          <div class="card fade-in-up" style="margin-bottom:2rem;">
            <div class="card-body">
              <h3 class="section-title" style="margin-bottom:1.5rem;">品牌代言</h3>
              <div class="grid-2">
                <div class="card hover-lift fade-in-up" v-for="(endorsement, i) in endorsements" :key="'e-'+i" style="overflow:hidden;">
                  <div style="height:100px;background:linear-gradient(135deg,#2d2d2d,#1a1a1a);display:flex;align-items:center;justify-content:center;position:relative;">
                    <span style="font-size:2rem;font-weight:700;letter-spacing:0.1em;color:rgba(255,255,255,0.6);">{{ endorsement.brand }}</span>
                  </div>
                  <div class="card-body" style="padding:1.5rem;">
                    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:0.5rem;">
                      <h4 class="card-title" style="font-size:1.1rem;margin:0;font-weight:700;">{{ endorsement.brand }}</h4>
                      <span class="card-tag" style="padding:0.2rem 0.6rem;border-radius:0;font-size:0.75rem;font-weight:600;color:var(--primary);background:rgba(255,107,53,0.1);">{{ endorsement.role }}</span>
                    </div>
                    <div style="display:flex;gap:0.5rem;margin-bottom:0.75rem;">
                      <span style="font-size:0.8rem;color:var(--text-muted);">{{ endorsement.year }}</span>
                      <span style="font-size:0.8rem;color:var(--primary);font-weight:500;">· {{ endorsement.type }}</span>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <!-- Official Positions Section -->
          <div class="card fade-in-up">
            <div class="card-body">
              <h3 class="section-title" style="margin-bottom:1.5rem;">官方身份与荣誉</h3>
              <div style="display:flex;flex-wrap:wrap;gap:1rem;">
                <div v-for="(position, i) in officialPositions" :key="'p-'+i"
                  style="flex:1;min-width:280px;padding:1.5rem;background:linear-gradient(135deg,rgba(255,107,53,0.05),rgba(212,168,83,0.05));border:1px solid rgba(255,107,53,0.15);border-radius:0;">
                  <div style="display:flex;align-items:center;gap:0.75rem;margin-bottom:0.75rem;">
                    <div style="width:40px;height:40px;border-radius:0;background:linear-gradient(135deg,var(--red-glow),var(--blue-glow));display:flex;align-items:center;justify-content:center;color:#fff;font-weight:700;font-size:0.9rem;">{{ position.icon }}</div>
                    <div>
                      <h4 style="font-size:1rem;font-weight:700;margin:0;">{{ position.title }}</h4>
                      <span style="font-size:0.8rem;color:var(--text-muted);">{{ position.year }}</span>
                    </div>
                  </div>
                  <p style="font-size:0.9rem;color:var(--text-muted);line-height:1.6;margin:0;">{{ position.description }}</p>
                </div>
              </div>
            </div>
          </div>
  ...
  data() {
    return {
      ...
      endorsements: window.SiteData.profile.endorsements || [],
      officialPositions: [
        { title: 'UNICEF韩国委员会亲善大使', year: '2024年9月至今', icon: 'U', description: '联合国儿童基金会韩国委员会亲善大使，UNICEF荣誉俱乐部最年轻会员，累计捐赠2亿韩元用于儿童公益项目。' },
        { title: '韩法建交140周年名誉大使', year: '2026年3月', icon: 'F', description: '与全智贤共同担任韩法建交140周年名誉大使，促进韩法文化交流。' },
        { title: 'Launchmetrics全球艺人媒体影响力第一', year: '2025年', icon: 'M', description: '媒体影响力价值（MIV）达20.1M，连续两年全球艺人第一。' }
      ]
    };
  },
            ...
            <div class="card hover-lift fade-in-up" v-for="(item, i) in behind" :key="i">
              ...
              <div style="padding:2rem;">
                <div style="display:flex;align-items:center;gap:0.75rem;margin-bottom:1rem;">
                  <div style="width:40px;height:40px;border-radius:0;background:linear-gradient(135deg,#FF6B35,#D4A853);display:flex;align-items:center;justify-content:center;">
                ...
                <div style="height:160px;display:flex;align-items:center;justify-content:center;background:linear-gradient(135deg,var(--bg),var(--bg-secondary));border-radius:0;margin-bottom:1rem;">
                  <span style="font-size:3rem;opacity:0.4;">·</span>
                </div>
