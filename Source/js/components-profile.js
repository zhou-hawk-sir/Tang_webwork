  ...
  template: `
    <div class="page-content">
      ...
      <section class="content-section">
        <div class="container">
          <!-- Profile Card -->
          <div class="card fade-in-up" style="padding:32px;text-align:center;border-radius:0;">
            ...
            <div class="profile-stats" style="display:grid;grid-template-columns:repeat(4,1fr);gap:16px;margin:24px 0;">
              <div class="stat-item" style="text-align:center;padding:12px;background:var(--gray-50);border-radius:0;">
            ...
            <div class="card-body">
              ...
              <div style="display:flex;flex-wrap:wrap;gap:0.75rem;margin-top:1rem;">
                <span v-for="(nick, i) in basic.nickname" :key="i"
                  style="display:inline-block;padding:0.5rem 1.25rem;background:linear-gradient(135deg,rgba(255,107,53,0.1),rgba(212,168,83,0.1));border:1px solid rgba(255,107,53,0.3);border-radius:0;font-size:0.95rem;color:var(--primary);font-weight:500;">
                  {{ nick }}
                </span>
              </div>
            </div>
            ...
            <template v-for="(group, gi) in yearGroups" :key="gi">
              <div class="timeline-item fade-in-up" v-for="(item, ii) in group.items" :key="gi + '-' + ii">
                ...
                <div class="timeline-content">
                  ...
                  <span class="timeline-tag" style="display:inline-block;margin-top:0.75rem;padding:0.25rem 0.75rem;border-radius:0;font-size:0.8rem;font-weight:500;color:var(--primary);background:rgba(255,107,53,0.1);">
                    {{ item.tag }}
                  </span>
                </div>
              </div>
      ...
      <section class="content-section">
        <div class="container">
          ...
          <div class="fade-in-up" style="margin-top:3rem;">
            ...
            <div v-for="(quote, i) in quotes" :key="i" style="margin-bottom:1.5rem;padding:1.5rem;background:linear-gradient(135deg,rgba(255,107,53,0.05),rgba(212,168,83,0.05));border-left:4px solid var(--primary);border-radius:0;">
...
window.Components.Awards = {
            ...
            <div class="card-body" style="text-align:center;padding:1.5rem;">
              <div style="display:flex;justify-content:center;gap:2rem;flex-wrap:wrap;">
                <div style="text-align:center;">
                  ...
                  <span style="font-size:0.9rem;color:var(--text-muted);display:block;margin-top:0.25rem;">项荣誉记录</span>
                </div>
                <div style="text-align:center;">
                  <span style="font-size:2rem;font-weight:700;color:#22c55e;">{{ winCount }}</span>
                  <span style="font-size:0.9rem;color:var(--text-muted);display:block;margin-top:0.25rem;">项获奖</span>
                </div>
                <div style="text-align:center;">
                  <span style="font-size:2rem;font-weight:700;color:#3b82f6;">{{ nominateCount }}</span>
                  <span style="font-size:0.9rem;color:var(--text-muted);display:block;margin-top:0.25rem;">项提名</span>
                </div>
              </div>
            </div>
          </div>
              ...
              <table class="data-table" style="width:100%;border-collapse:collapse;">
                <thead>
                  <tr style="border-bottom:2px solid var(--border);">
                    ...
                    <th style="padding:0.75rem 1rem;text-align:left;font-weight:600;color:var(--text-muted);">备注</th>
                  </tr>
                </thead>
                <tbody>
                  <tr v-for="(item, i) in filteredItems" :key="i" style="border-bottom:1px solid var(--border);">
                    ...
                    <td v-if="item.note" style="padding:0.75rem 1rem;font-size:0.85rem;color:var(--text-muted);font-style:italic;">{{ item.note }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
  ...
  computed: {
    ...
    winCount() {
      return this.filteredItems.filter(a => a.result === '获奖').length;
    },
    nominateCount() {
      return this.filteredItems.filter(a => a.result === '提名').length;
    }
  },
        ...
        <div class="container">
          ...
          <div class="fade-in-up" style="text-align:center;margin-bottom:2rem;">
            <div style="display:inline-flex;align-items:flex-end;gap:4px;height:48px;">
              <span v-for="n in 12" :key="n" style="display:inline-block;width:5px;border-radius:0;background:var(--grad-warm);animation:visualizerBar 0.8s ease-in-out infinite alternate;"
            ...
            <div class="card hover-lift" v-for="(style, i) in styles" :key="i">
              <div class="card-body" style="padding:2rem;">
                <div style="display:flex;align-items:center;gap:1rem;margin-bottom:1rem;">
                  <div style="width:48px;height:48px;border-radius:0;background:linear-gradient(135deg,#FF6B35,#D4A853);display:flex;align-items:center;justify-content:center;color:#fff;font-size:1.5rem;font-weight:700;">{{ i + 1 }}</div>
          ...
          <div class="fade-in-up" style="margin-top:3rem;">
            ...
            <div style="display:flex;flex-wrap:wrap;gap:0.75rem;">
              <span v-for="(stage, i) in highlightStages" :key="i"
                style="display:inline-flex;align-items:center;gap:0.5rem;padding:0.75rem 1.5rem;background:linear-gradient(135deg,rgba(255,107,53,0.1),rgba(212,168,83,0.1));border:1px solid rgba(255,107,53,0.2);border-radius:0;font-size:0.95rem;font-weight:500;color:var(--primary);">
            ...
            <div class="card hover-lift" v-for="(lang, i) in languages" :key="i">
              <div class="card-body" style="padding:2rem;">
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1rem;">
                  ...
                  <span class="card-tag" style="padding:0.25rem 0.75rem;border-radius:0;font-size:0.85rem;font-weight:600;color:var(--primary);background:rgba(255,107,53,0.1);">{{ lang.level }}</span>
                </div>
                ...
                <div style="background:var(--gray-100);border-radius:0;height:8px;overflow:hidden;">
                  <div style="height:100%;border-radius:0;background:var(--grad-warm);transition:width 0.8s ease;"
        ...
        <div class="container">
          ...
          <div class="grid-2 fade-in-up">
            <div class="card hover-lift" v-for="(style, i) in styles" :key="i">
              <div class="card-body" style="padding:2rem;">
                <div style="display:flex;align-items:center;gap:1rem;margin-bottom:1rem;">
                  <div style="width:56px;height:56px;border-radius:0;background:linear-gradient(135deg,rgba(255,107,53,0.15),rgba(212,168,83,0.15));display:flex;align-items:center;justify-content:center;font-size:1.5rem;">·</div>
            ...
            <div class="card-body" style="text-align:center;padding:2rem;">
              ...
              <div style="display:flex;flex-wrap:wrap;justify-content:center;gap:2rem;">
                <div v-for="(brand, i) in brands" :key="i"
                  style="display:flex;flex-direction:column;align-items:center;gap:0.5rem;padding:1.5rem 2.5rem;background:var(--bg);border:1px solid var(--border);border-radius:0;min-width:160px;">
